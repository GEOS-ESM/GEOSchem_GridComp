...
!
    ...
    LOGICAL               :: FriendDyn, FriendTurb, Found
    ...
    DO WHILE( INDEX( LINE, 'TRANSPORT MENU' ) .le. 0) 
      ...
      IF ( INDEX( LINE, 'Species name' ) > 0 ) THEN
         ...
         IF ( N > 1 ) THEN
            ...
            ENDIF
         ENDIF
         IF (       FriendDyn .AND.       FriendTurb ) MYFRIENDLIES = 'DYNAMICS:TURBULENCE'
         IF (       FriendDyn .AND. .NOT. FriendTurb ) MYFRIENDLIES = 'DYNAMICS'
         IF ( .NOT. FriendDyn .AND.       FriendTurb ) MYFRIENDLIES = 'TURBULENCE'
         IF ( .NOT. FriendDyn .AND. .NOT. FriendTurb ) MYFRIENDLIES = 'GEOSCHEMCHEM'

         ! Get long name
         iName = TRIM(SUBSTRS(1))
         CALL Spc_Info ( am_I_Root = am_I_Root, iName=iName, KppSpcID=-1, &
                         oFullName = FullName, oFormula = Formula, &
         ...
         call MAPL_AddInternalSpec(GC, &
              ...
              LONG_NAME          = TRIM(FullName)//'_mass_mixing_ratio_total_air', &
              UNITS              = 'kg kg-1',                &
...
!!!              PRECISION          = ESMF_KIND_R8,             &
         ...
         ! verbose
         if(am_I_Root) write(*,*) 'GCC added to internal: TRC_'//TRIM(SUBSTRS(1)), '; Friends: ', TRIM(MYFRIENDLIES)
      ENDIF
    ENDDO
