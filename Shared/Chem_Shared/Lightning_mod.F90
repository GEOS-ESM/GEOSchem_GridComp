...
subroutine getLightning (GC, ggState, CLOCK, &
  ...
  logical,                  intent(in)  :: useImportedCape    ! Whether to use CAPE, INHB and BYNCY from Import State, or compute them locally
  real, dimension(:,:),     intent(in)  ::  CAPE_IMPORT
  real, dimension(:,:),     intent(in)  ::  INHB_IMPORT
  real, dimension(:,:,:),   intent(in)  :: BYNCY_IMPORT
...
!!         ZLE_ZERO is height above surface,   z-index 0..72
  ...
  if (flash_source_enum == FLASH_SOURCE_LOPEZ) then
     ...
     ! useImportedCape - in GCM this means to use the values from MOIST
     ! useImportedCape - in CTM this means to use the values from ExtData or CtmEnv (not presently available)
     if (useImportedCape) then
       CAPE  =  CAPE_IMPORT
       INHB  =  INHB_IMPORT
       BYNCY = BYNCY_IMPORT
...
subroutine read_lightning_config ( im, jm, rcfilen, flash_source_enum,   &
  ...
  logical,                     intent(out)  :: useImportedCape
  integer, optional,           intent(out)  :: RC
  ...
  call ESMF_ConfigGetAttribute(esmfConfig, useImportedCape,    &
                               Label    = "useImportedCape:",  &
                               Default  =  .TRUE.,      __RC__ )
