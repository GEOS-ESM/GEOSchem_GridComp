...
!  -------------------------------------------
   ...
   if (STATUS /= 0) then
     if (mapl_am_i_root()) print*,'SSng_GridComp_'//trim(COMP_NAME)//'.rc does not exist! loading SSng_GridComp_SS.data.rc instead'
...
! Local derived type aliases
    ...
    integer                             :: i, n_bins, nbins_test, aero_n, nCols
    integer                              :: instance
...
!   !Local
    ...
    real(kind = 8), dimension(:,:,:), allocatable            :: ext, ssa, asy  ! (lon:,lat:,lev:,band:)
