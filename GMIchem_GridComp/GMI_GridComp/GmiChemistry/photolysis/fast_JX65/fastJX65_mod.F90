...
! !PUBLIC MEMBER FUNCTIONS:
      public :: controlFastJX65
      public :: initializeFastJX65
      public :: getQAA_RAAinFastJX65
!
      INTEGER, PARAMETER :: numScat = 5
...
!
! !HISTORY
! 2016.05.26  Luke Oman & Huisheng Bian
!             Finished the FastJX 6.5 implementation
!
!EOP
!---------------------------------------------------------------------------
      contains
...
! !INPUT PARAMETERS:
      ...
      logical, intent(in)  :: do_clear_sky       ! no cloud effects
      real*8 , intent(in) :: tau_clw_ij(k1:k2)   ! optical thickness for liquid cloud
      real*8 , intent(in) :: tau_cli_ij(k1:k2)   ! optical thickness for ice cloud
...
!! start-by-hbian
!      do ik = k1, k2
!        if( tau_clw_ij(ik) .ge. tau_cli_ij(ik)) then
!          CLDNDX(I_, J_, ik) = 9
!        else
!          CLDNDX(I_, J_, ik) = 13
!        endif
...
! please define these variables and input their values
      !CLDW(I_, J_, 1:L_)  = 0.0d0 ! tau_clw_ij(k1:k2)
      !CLDW(I_, J_, L1_)   = 0.d0
      !CLDI(I_, J_, 1:L_)  = 0.0d0 ! tau_cli_ij(k1:k2)
      !CLDI(I_, J_, L1_)   = 0.d0
      if ( do_clear_sky ) then
        CLDW(I_, J_, 1:L_)  = 0.d0
        CLDW(I_, J_, L1_)   = 0.d0
        CLDI(I_, J_, 1:L_)  = 0.d0
        CLDI(I_, J_, L1_)   = 0.d0
      else
        CLDW(I_, J_, 1:L_)  = tau_clw_ij(k1:k2)
        CLDW(I_, J_, L1_)   = 0.d0
        CLDI(I_, J_, 1:L_)  = tau_cli_ij(k1:k2)
        CLDI(I_, J_, L1_)   = 0.d0
      end if
      !if (londeg_i .ge. 70.d0 .and. londeg_i .lt. 72.d0 .and. &
      !    latdeg_j .ge. 44.0d0 .and. latdeg_j .lt. 46.d0) then

      !CLDWNDX(I_, J_, k1:k2) = 9
      !CLDINDX(I_, J_, k1:k2) = 13
      CLDWNDX(I_, J_, 1:L1_) = 9
      CLDINDX(I_, J_, 1:L1_) = 13
...
! endBian, 11/21/2011
      ...
      OPTDUST(L1_,:)     = 0.0d0 ! ODMDUST_ij(k1:k2,:)
      OPTAER (L1_,:)     = 0.0d0 ! ODAER_ij  (k1:k2,:)

      ! Set pressure at the center of grid box
      PJ(1)                = pctm_ij
      PJ(2:L1_)            = press3e_ij(k1:k2)
      PJ(L1_+1)            = 0.0d0
      ...
      ! Sets up clouds
      !---------------
      !call SET_CLD (GMTAU)   <------- This is empty at this time
...
!-----------------------------------------------------------------------
      ...
      ! call SET_PROF   <--- does not exist. Do we have to write it?

      !--------------------------------------
      ! Compute the photolysis rate constants
      !--------------------------------------

      ! GMTAU: Number days of simulation (in)
      ! IDAY:  Current day (in)
      ! ILNG: (in)
      ! JLAT: (in)
      ! SOLF:  Solar flux (out)
      ! SZA_ij: Solar zenith angle in degree (in)
      ! U0:    cosine of the solar zenith angle or cos(SZA) (out)
      ...
      end subroutine controlFastJX65
!EOC
!---------------------------------------------------------------------------
!BOP
!
! !IROUTINE: initializeFastJX65
!
! !INTERFACE:
!
...
!      allocate(OPTDUSt(1:L_,NSADdust))
      allocate(optDust(1:L1_,NSADdust))
      optDust  = 0.0d0
!      allocate(OPTAER (1:L_,NSADaer*nrh_b))
      ...
      allocate (CLDWNDX(I_,J_,L1_))
      allocate (CLDINDX(I_,J_,L1_))
...
!      allocate(AER1P_aero(1:L_,1:NSADaer))
!      AER1P_aero = 0.0d0
!      allocate(AER1P_dust(1:L_,1:NSADdust))
!      AER1P_dust = 0.0d0
...
! bian end
      ...
      end subroutine initializeFastJX65
!EOC
!---------------------------------------------------------------------------
!BOP
!
! !IROUTINE: getQAA_RAAinFastJX65
!
! !INTERFACE:
!
...
!BOC
      ...
      end subroutine getQAA_RAAinFastJX65
!EOC
!---------------------------------------------------------------------------

!  >>>>>>>>>>>>>>>>current code revised to JX ver 6.5 (8/09)<<<<<<<<<<<<
!
! version 6.5
! >>>> redo J-values to be average over CTM layer (not mid-layer)
!
! version 6.4
...
!---Given the aerosol+cloud OD/layer in visible (600 nm) calculate how t
...
!-----------------------------------------------------------------------
      ...
      call OPMIE (DTAUX, POMEGAX, U0, RFL, AMF2, JXTRA, AVGF, FJTOP, &
...
!      write (6,  * ) 'fast-JX-(6.5)----PHOTOJ internal print: Atmosphere &
!&----'
...
!######################################################################
      subroutine RD_TJPL(rootProc,NJ1,NAMFIL)
!-----------------------------------------------------------------------
...
! endBian, 1/07/2012
      if (rootProc) then
         print *,'RD_TJPL   NJVAL NQQQ =',NJVAL,NQQQ
      endif
...
!---Read remaining species:  X-sections at 2 T's
      do J=1,NQQQ
! begBian, bugfix, 11/30/2011
!        read(NJ1,103) TITLEJ(J+3),TQQ(1,J+3),(QQQ(IW,1,J),IW=1,NWWW)
!        read(NJ1,103) TITLEJ2,TQQ(2,J+3),(QQQ(IW,2,J),IW=1,NWWW)
        read(NJ1,103) TITLEJ(J+3),TQQ(1,J+3),(QQQ(IW,1,J+3),IW=1,NWWW)
        read(NJ1,103) TITLEJ2,    TQQ(2,J+3),(QQQ(IW,2,J+3),IW=1,NWWW)
! endBian
        !if(TRIM(titlej(j+3)).eq."C3H6Ot") ncat_acetone_trop  = J+3  ! Added for GMI to catch trop acetone {AOO, 8/04}
        !if(TRIM(titlej(j+3)).eq."C3H6Os") ncat_acetone_stra     = J+3  ! Added for GMI to catch strat acetone {AOO, 8/04}
...
!---Read aerosol phase functions:
      ...
      if(NAA.gt.A_) then
!        write(6,350) NAA
        STOP " too many scat-data for Fast_JX"
      endif
! begBian, 11/30/2011
! note: change NK when using new table
...
! endBian
         ...
         enddo
            ...
            IF (rootProc) write(6,*)'Number of used wavelengths wrong:', W_
            stop
         endif
      endif

!-----------------------------
! End Tropospheric Treatment
!-----------------------------
...
!      do j=1,NJVAL
!        do k=1,JVN_
!          if(TRIM(JLABEL(k)) .eq. TRIM(TITLEJ(j))) jind(k)=j
!        enddo
!      enddo
!
!      do k=1,JVN_
!        if(JFACTA(k).eq.0.d0 .AND. rootProc)  &
!     &             write(6,*) 'RD_TJPL: Not using photolysis reaction ',k
!        if(jind(k).eq.0) then
!          if(JFACTA(k).eq.0.d0) then
!            jind(k)=1
!          else
!            write(6,*) 'RD_TJPL: Which J-rate for photolysis reaction ',k,' ?'
!            STOP "Fast_JX"
!          endif
!        endif
!      enddo
...
! 103 FORMAT(A7,F3.0,7E10.3/(10X,7E10.3))
  104 FORMAT(13x,i2)
...
subroutine EXTRAL (DTAUX, L1X, L2X, NX, JTAUMX, ATAU, ATAU0, &
 JXTRA)
!-----------------------------------------------------------------------
!
!    new version 6.1, add sub-layers (JXTRA) to thick cloud/aerosol laye
!    this version sets up log-spaced sub-layers of increasing thickness
!
...
!---The log-spacing parameters have been tested for convergence and chos
!---  to be within 0.5% for ranges OD=1-500, rflect=0-100%, mu0=0.1-1.0
!---  use of ATAU = 1.18 and min = 0.01, gives at most +135 pts for OD=1
...
!
!---Divide thick layers to achieve better accuracy in the scattering cod
!---In the original fast-J, equal sub-layers were chosen, this is wastef
!---and this new code (ver 5.3) uses log-scale:
!---        Each succesive layer (down) increase thickness by ATAU > 1
!---        e.g., if ATAU = 2, a layer with OD = 15 could be divided int
...
!     POMEGAX(1:8,1:L1_,1:W_) = scattering phase fn (multiplied by s-s a
!     U0  = cos (SZA)
!     RFL(1:W_) = Lambertian albedo of surface
!     AMF2(1:2*L1_+1,1:2*L1_+1) = air mass factor (I,L)=wt of layer-I to
!        AMF2 now does both edges and middle of CTM layers
!     JXTRA(1:L1_) = number 0:J = no. of additional levels to be inserte
! out:
!     FJACT(1:L_,1:W_) = mean actinic flux(diff+direct) at std CTM level
!  (new ver 5.7 diagnostics for fluxes, deposition)  fluxes 'down' are <
...
!        this connects with FJBOT = FJFLX(0) & FJTOP = FJFLX(L_+1) (not
!     FLXD(1:L_+1,1:W_) = solar flux deposited in layer L (includes lyr
...
!     TTAU     Optical depth of air vertically above each point (to top
...
!  to the surface (down in 'LZ', up in 'L2'). This allows ztau and pomeg
...
!  remember L2=1 is surface of CTM, but last layer (LZ) in scattering co
!  there are twice the number of layers in the LZ arrays (2*L2_ + 2*JADD
...
!           ***JXTRA(1:L2_+1) is calculated based on the aerosol+cloud O
!     JADDLV(L2)  Number of new levels actually added at each wavelength
!            where JADDLV = 0 when there is effectively no FTAU2
!     JADDTO(L2)   Total number of new levels to add to and above level
!     JNDLEV(L) = L2 index that maps on CTM mid-layer L
!
!---JADDLV(L2=1:L2_) = number of levels to add between TTAU2(L2) and TTA
...
!######################################################################
!-----------------------------------------------------------------------
subroutine BLKSLV (FJ, POMEGA, FZ, ZTAU, ZFLUX, RFL, PM, PM0, &
 FJTOP, FJBOT, ND)
!-----------------------------------------------------------------------
 ...
 * WT (4) * EMU (4)
SUMB = RR (1, ND, K) * WT (1) * EMU (1) + RR (2, ND, K) * WT (2) &
...
!######################################################################
!-----------------------------------------------------------------------
      subroutine GEN_ID (POMEGA, FZ, ZTAU, ZFLUX, RFL, PM, PM0, B, CC, &
 AA, A, H, C, ND)
!-----------------------------------------------------------------------
...
!---------upper boundary:  2nd-order terms
      ...
      do I = 1, M_
         ...
         SUM2 = POMEGA (1, L2) * PM (I, 1) * PM0 (1) + POMEGA (3, L2) &
             * PM (I, 3) * PM0 (3) + POMEGA (5, L2) * PM (I, 5) * PM0 (5) &
             + POMEGA (7, L2) * PM (I, 7) * PM0 (7)
         SUM1 = POMEGA (2, L1) * PM (I, 2) * PM0 (2) + POMEGA (4, L1) &
             * PM (I, 4) * PM0 (4) + POMEGA (6, L1) * PM (I, 6) * PM0 (6) &
             + POMEGA (8, L1) * PM (I, 8) * PM0 (8)
      ...
      do I = 1, M_
         do J = 1, I
            ...
            SUM2 = POMEGA (1, L2) * PM (I, 1) * PM (J, 1) + POMEGA (3, L2) &
                 * PM (I, 3) * PM (J, 3) + POMEGA (5, L2) * PM (I, 5) * PM (J, 5) &
                 + POMEGA (7, L2) * PM (I, 7) * PM (J, 7)
            SUM1 = POMEGA (2, L1) * PM (I, 2) * PM (J, 2) + POMEGA (4, L1) &
                 * PM (I, 4) * PM (J, 4) + POMEGA (6, L1) * PM (I, 6) * PM (J, 6) &
