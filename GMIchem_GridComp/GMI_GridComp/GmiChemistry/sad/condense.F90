...
! to be calculated
      ...
      do 11 ik = k1,k2
         ...
         if(hno3cond(il,ij,ik).gt.MIN_VALUE)then
            go = .true.
            goto 12
         endif
...
! and upper alt of condensed phase
      ...
      do 13 ik = k1,k2
         ...
         if(hno3cond(il,ij,ik).gt.MIN_VALUE  &
     &      .or.h2ocond(il,ij,ik).gt.MIN_VALUE)then
...
! Therefore, sediment hno3 using h2o particle size distribution.
         ...
         if(h2ocond(il,ij,ik).gt.MIN_VALUE)then
...
!         ---------------------------
     ...
     &      hno3cond(il,ij,ik) + ((prod - loss) / dens)
          ...
          if (hno3cond(il,ij,ik) .lt. MIN_VALUE) then
              hno3cond(il,ij,ik) = 0.0d0
          end if

          if ((h2ocond(il,ij,ik)   .gt. MIN_VALUE) .or.  &
     &        (h2ocond(il,ij,ik+1) .gt. MIN_VALUE)) then
...
!           -----------------------------------------------------------
     ...
     &        h2ocond(il,ij,ik) + (loss / dens) - prod
            ...
            if (h2ocond(il,ij,ik) .lt. MIN_VALUE) then
                h2ocond(il,ij,ik) = 0.0d0
            end if
