...
class PYCHEM:
    ...
    def pchem(
        ...
        python_state_data["ple"] = self._f2py.fortran_to_python(ple, dim=ple_dim_sizes, rank=ple_rank)

        python_state_data["mncv"] = self._f2py.fortran_to_python(mncv, dim=mncv_dim_sizes, rank=mncv_rank)

        python_state_data["lats"] = self._f2py.fortran_to_python(lats, dim=lats_dim_sizes, rank=lats_rank)
        ...
        python_state_data["tropp"] = self._f2py.fortran_to_python(tropp, dim=tropp_dim_sizes, rank=tropp_rank)
        ...
        python_state_data["O3VMR"] = self._f2py.fortran_to_python(O3VMR, dim=O3VMR_dim_sizes, rank=O3VMR_rank)
        python_state_data["OX"] = self._f2py.fortran_to_python(OX, dim=OX_dim_sizes, rank=OX_rank)
        python_state_data["ZTH"] = self._f2py.fortran_to_python(ZTH, dim=ZTH_dim_sizes, rank=ZTH_rank)

        python_state_data["CH4"] = self._f2py.fortran_to_python(CH4, dim=CH4_dim_sizes, rank=CH4_rank)
        python_state_data["N2O"] = self._f2py.fortran_to_python(N2O, dim=N2O_dim_sizes, rank=N2O_rank)
        python_state_data["CFC11"] = self._f2py.fortran_to_python(CFC11, dim=CFC11_dim_sizes, rank=CFC11_rank)
        python_state_data["CFC12"] = self._f2py.fortran_to_python(CFC12, dim=CFC12_dim_sizes, rank=CFC12_rank)
        python_state_data["HCFC22"] = self._f2py.fortran_to_python(
            HCFC22, dim=HCFC22_dim_sizes, rank=HCFC22_rank
        )
        python_state_data["H2O"] = self._f2py.fortran_to_python(H2O, dim=H2O_dim_sizes, rank=H2O_rank)
        python_state_data["O3"] = self._f2py.fortran_to_python(O3, dim=O3_dim_sizes, rank=O3_rank)
        python_state_data["O3PPMV"] = self._f2py.fortran_to_python(
            O3PPMV, dim=O3PPMV_dim_sizes, rank=O3PPMV_rank
        )
        python_state_data["AOA"] = self._f2py.fortran_to_python(AOA, dim=AOA_dim_sizes, rank=AOA_rank)
